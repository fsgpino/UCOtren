...
- (void)inicioDelView
{
    ...
    NSString *fileName = [NSString stringWithFormat:@"tramo-cordoba-rabanales.plist"];
    NSString *filePath = [documentDirectory stringByAppendingPathComponent:fileName];
    //
    NSInteger statusRC = 1;
    NSInteger statusCR = 1;
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath])
    {
        NSLog(@"El horario existe.");
    }else{
        statusRC = [self actualizaHorario:@"rabanales-cordoba"];
        statusCR = [self actualizaHorario:@"cordoba-rabanales"];
        if (((statusRC==1)||(statusRC==2))&&((statusCR==1)||(statusCR==2)))
        {
            NSLog(@"El horario descargado.");
        }else{
            NSLog(@"#ERROR#: No hay horario y tampoco conexion.");
        }
    }
    //
    if ((statusRC==0)||(statusCR==0))
    {
...
-(IBAction)recargarHorario:(id)sender {
    ...
    if ((statusCR==2)&&(statusRC==2))
    {
        ...
        if (((statusCR==1)&&(statusRC==1))&&((statusCR==1)&&(statusRC==2))&&((statusCR==2)&&(statusRC==1)))
        {
